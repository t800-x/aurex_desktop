use std::{collections::VecDeque, sync::{Arc, OnceLock}, time::Duration};
...
pub fn track_progress(app_handle: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            player_state.update( move |player| {
...
#[tauri::command]
#[specta::specta]
pub async fn load(state: tauri::State<'_, ManagedPlayer>, track: FullTrack) -> Result<AudioPlayer, String> {
    let audio_engine = audio_player().lock().await;
    _ = audio_engine.clone().load(&track.track.file_path).await;
...
#[tauri::command]
#[specta::specta]
pub async fn play_list(state: tauri::State<'_, ManagedPlayer>, list: Vec<FullTrack>, mut index: usize) -> Result<AudioPlayer, String> {
    let mut player = state.get().await;
    player.queue.clear();
...
#[tauri::command]
#[specta::specta]
pub async fn play_next(state: tauri::State<'_, ManagedPlayer>, track: FullTrack) -> Result<AudioPlayer, String> {
    state.update(|player| {
...
#[tauri::command]
#[specta::specta]
pub async fn add_to_queue(state: tauri::State<'_, ManagedPlayer>, track: FullTrack) -> Result<AudioPlayer, String> {
...
#[tauri::command]
#[specta::specta]
pub async fn change_queue_index(state: tauri::State<'_, ManagedPlayer>, old_index: usize, mut new_index: usize) -> Result<AudioPlayer, String> {
