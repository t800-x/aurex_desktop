<script lang="ts">
  ...
  let playlist_id = $derived(
    Number(router.current.toString().match(/pl-(\d+)/)?.[1]),
  );
...
<div style:display={displayMode} class:hidden class="playlistPage page">
  <div class="topInfo">
    {#if imgSrc !== null}
    ...
    <div class="playlistInfo">
      ...
      <div class="extraInfo">
        {tracks.length}
        {tracks.length === 1 ? "song" : "songs"} · {formatDuration(
          totalDuration,
        )}
      </div>
