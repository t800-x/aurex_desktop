...
<div class="artistView">
  {#if isThereSomethingToDisplay}
    <VList
      ...
      getKey={(album: Album, _: any) => album.id}
    >
      {#snippet children(album: Album, index: number)}
        ...
        <div class="album">
          {#if album.album_art !== null}
            <img
              class="albumCover"
              src={convertFileSrc(album.album_art)}
              alt=""
            />
          {:else}
          ...
          <div class="tracklist">
            <div
              class="title"
              style=" margin: 20px; width: calc(100% - 40px); display: flex;"
            >
              <div>
                <div style="font-weight: 500;">{album.title}</div>
                <div
                  style="font-weight: 500; font-size: 13px; color: var(--color-navbar-label);"
                >
            ...
            <div class="tracks" style="padding: 20px; width: 100%;">
              ...
              {#await commands.getAlbumTracks(Number(album.id)) then tracks}
