...
<dialog
  ...
  onclick={(e) => {
    if (e.target === dialog) router.closeDeletePlaylistDialog();
  }}
>
  <div style="padding: 24px;">
    ...
    <p style="margin-top: 10px; font-size: 14px; color: rgba(255,255,255,0.6);">
      Are you sure you want to delete <strong style="color: white;"
        >"{router.pendingDeletePlaylist?.name}"</strong
  ...
  >
    <div class="bottomButtonContainer">
      <button
        class="bottomButton deleteBtn"
        onclick={async () => {
          await commands.deletePlaylist(
            Number(router.pendingDeletePlaylist!.id),
          );
    ...
    <div class="bottomButtonContainer">
      <button
        class="bottomButton"
        onclick={() => router.closeDeletePlaylistDialog()}>Cancel</button
      >
    </div>
  </div>
</dialog>
