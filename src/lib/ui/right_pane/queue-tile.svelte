<script lang="ts">
  ...
  let shift = $derived(() => {
    ...
    if (draggedFrom < draggedTo) {
      ...
      if (currentIndex > draggedFrom && currentIndex <= draggedTo)
        return -tileHeight;
    } else {
      // dragging upward: items between to and from-1 shift down
      if (currentIndex >= draggedTo && currentIndex < draggedFrom)
        return tileHeight;
    }
    return 0;
  });
  ...
  onMount(() => {
    const cleanup = [
      draggable({
        ...
        onDragStart: () => {
          draggedFrom = currentIndex;
          draggedTo = currentIndex;
        },
        onDrop: () => {
          draggedFrom = null;
          draggedTo = null;
...
<ContextMenu.Root>
  <ContextMenu.Trigger>
    <div
      ...
      onpointerenter={() => {
        if (draggedFrom !== null) draggedTo = currentIndex;
      }}
    ...
    >
      ...
      <button
        class="removeBtn"
        onclick={(e) => commands.removeFromQueue(index)}
      >
        <svg
  ...
  <QueueContextMenu {track} {index} {lastIndex} />
</ContextMenu.Root>
