...
<ContextMenu.Content onInteractOutside={(e) => e.stopPropagation()}>
  ...
  <ContextMenu.Item onclick={() => playList()}>Play from here</ContextMenu.Item>
  <ContextMenu.Item onclick={async () => commands.playNext(track)}
    >Play Next</ContextMenu.Item
  >
  <ContextMenu.Item onclick={async () => commands.addToQueue(track)}
